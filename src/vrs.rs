...
use rand::{Rng, distr::Uniform, random, rng, seq::SliceRandom};
...
// pub fn softmax_optimized(z: &Array2<f32>) -> Array2<f32> {
//     // let shape = z.raw_dim();
//     let mut result = z.clone(); // in-place safe
//
//     result
...
fn forward_prop_bak(
    ...
    let a1 = z1.clone();
    relu(&mut z1); // Apply ReLU activation to z1: \( a_1 = \text{ReLU}(z_1) \)
...
fn forward_prop(
    ...
    let mut z1 = w1.dot(x);
    if b1.shape()[1] == 1 {
        // Fast broadcast along columns
        z1 += &b1.broadcast(z1.raw_dim()).unwrap();
    } else {
        // Manual fallback for shape mismatch
        z1.axis_iter_mut(Axis(1))
            .zip(b1.axis_iter(Axis(1)))
            .for_each(|(mut col, bias_col)| {
                col += &bias_col;
            });
    }
    ...
    // z2 = W2 · A1 + b2
    let mut z2 = w2.dot(&a1);
    if b2.shape()[1] == 1 {
        z2 += &b2.broadcast(z2.raw_dim()).unwrap();
    } else {
        z2.axis_iter_mut(Axis(1))
            .zip(b2.axis_iter(Axis(1)))
            .for_each(|(mut col, bias_col)| {
                col += &bias_col;
            });
    }
...
/// for each sample.
/// Fast and flexible one-hot encoder with auto class count and column-major output
fn one_hot(y: &ArrayView1<f32>) -> Array2<f32> {
    let max_class = y
        .into_par_iter()
        .cloned()
    ...
    let mut one_hot = Array2::<f32>::zeros((num_classes, batch_size));

    // Parallel assignment to one-hot positions
    Zip::from(y)
        .and(one_hot.axis_iter_mut(Axis(1)))
        .par_for_each(|&label, mut col| {
            col[label as usize] = 1.0;
        });
...
// fn backward_prop_new(
//     mut z1: Array2d, // Will be used in-place for ReLU derivative
//     a1: &Array2d,
//     a2: &Array2d,
//     w2: &Array2d,
//     x: &ArrayView2<f32>,
...
//
//     // Precomputed one-hot encoding with static num_classes = 10
...
//     deriv_relu(&mut z1); // Apply ReLU' in-place to z1
//     let dz1 = w2.t().dot(&dz2) * z1;
//
...
fn update_params(
    ...
    w1.scaled_add(-alpha, dw1);
    b1.scaled_add(-alpha, db1);
    w2.scaled_add(-alpha, dw2);
    b2.scaled_add(-alpha, db2);
}
...
fn get_predictions(a2: &Array2<f32>) -> Vec<usize> {
    a2.axis_iter(Axis(1))
        .into_par_iter()
        .map(|column| {
            column
                ...
                .map(|(i, _)| i)
                .unwrap_or(0) // fallback if all values are NaN
        })
        .collect()
}
...
fn get_accuracy(predictions: Vec<usize>, y: &ArrayView1<f32>) -> f32 {
    ...
    Zip::from(predictions.as_slice()).and(y).par_fold(
        || 0.0f32,
...
fn gradient_descent(
    ...
    for iter in 0..iters {
        // Shuffle indices for random batches each epoch
        let mut indices: Vec<usize> = (0..total_samples).collect();
        indices.shuffle(&mut rng());

        // Compute gradients for each batch in parallel
        let batch_grads: Vec<(Array2d, Array2d, Array2d, Array2d)> = (0..num_batches)
            .into_par_iter()
            .map(|batch_idx| {
                ...
                // Slice batches efficiently using selected indices
                let x_batch = x.select(Axis(1), batch_indices);
                let y_batch = y.select(Axis(0), batch_indices);

                // Forward and backward pass using current params
                let (z1, a1, _z2, a2) = forward_prop(&w1, &b1, &w2, &b2, &x_batch.view()).into();
                // backward_prop(z1, &a1, &a2, &w2, &x_batch.view(), &y_batch.view())
                back_prop(z1, &a1, &a2, &w2, &x_batch.view(), &y_batch.view()).into()
            })
            .collect();

        // Aggregate gradients by summing them all
        let mut dw1 = Array2d::zeros(w1.dim());
        let mut db1 = Array2d::zeros(b1.dim());
        let mut dw2 = Array2d::zeros(w2.dim());
        let mut db2 = Array2d::zeros(b2.dim());

        for (d1, db_1, d2, db_2) in batch_grads {
            dw1 += &d1;
            db1 += &db_1;
            dw2 += &d2;
            db2 += &db_2;
        ...
        // Average gradients
        let scale = 1.0 / (num_batches as f32);
        dw1 *= scale;
        db1 *= scale;
        dw2 *= scale;
        db2 *= scale;

        // Update params **in place** (best for performance)
        w1.scaled_add(-alpha, &dw1);
        b1.scaled_add(-alpha, &db1);
        w2.scaled_add(-alpha, &dw2);
        b2.scaled_add(-alpha, &db2);

        // Optional: evaluate accuracy every 10 iterations
        if iter % 10 == 0 {
            let (_, _, _, a2_full) = forward_prop(&w1, &b1, &w2, &b2, &x.view()).into();
            let preds = get_predictions(&a2_full);
            let acc = get_accuracy(preds, y);
            println!("Iteration {}: Accuracy = {:.4}%", iter, acc * 100.0);
        }
    }
...
// fn evaluate_and_test_samples(
//     x_dev: &Array2<f32>,
//     y_dev: &ArrayView1<f32>,
//     w1: &Array2<f32>,
//     b1: &Array2<f32>,
//     w2: &Array2<f32>,
//     b2: &Array2<f32>,
//     num_samples_to_show: usize,
// ) {
//     // Forward pass on the whole dev set
//     let (_, _, _, a2) = forward_prop(w1, b1, w2, b2, x_dev);
//
//     // Get predictions for all samples
//     let predictions = get_predictions(&a2);
//
//     // Calculate overall accuracy
//     let accuracy = calculate_accuracy(&predictions, y_dev);
//     println!("Test Accuracy: {:.2}%", accuracy * 100.0);
//
//     // Show some individual samples randomly
//     let total_samples = x_dev.len_of(Axis(1));
//     let mut rng = thread_rng();
//
//     for _ in 0..num_samples_to_show {
...
//
//         // Optional: show image or more info
//         // let image = x_dev.slice(s![.., idx]);
//         // show_image(&image, pred, actual, idx);
//     }
// }
